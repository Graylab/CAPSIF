...
class pdb_to_interaction_file:
    def __init__(self,prot_file, out_dir = "./",check_done=True, verbose = 1, use_glycan=1,saveData=1):
    ...
    def run_me(self):
        ...
        if ((self.flag == True) or (self.carb_aa_distance_calc_and_save == 0)):
            ...
            #print(self.all_res_fixed_data[1][:5])
            self.combine_data()
            #print(self.all_res_fixed_data[1][:5])
            if self.save_data ==1:
                self.save_npz_pdb()
    ...
    def save_npz_pdb(self):
        ...
        '''
        np.savez(self.out_dir + "/" + self.pdb_file_name + "_egnn_data.pdb.npz",
    ...
    def extract_CB_and_CA_xyz(self):
        ...
        for res in range(1, pose.size()+1):
            if pose.conformation().residue(res).is_protein():
                ...
                pdb_chain = pose.pdb_info().pose2pdb(res).split(' ')[1]
                input_mat = [ res,self.aa_seq.index(res1_name)+1,pdb_chain ]
...
dirs = ['test','test_af','train','val']

init_dir = "../dataset/"



done_data = done_data_recorder("./done_pdbs_egnn.txt",str)
for dir_type in dirs:
    pdb_dir = init_dir + dir_type + "/pdbs/"
    out_dir = init_dir + dir_type + "/g_npz/"
    #print(pdb_dir)
